...
for file in os.listdir(path + '/src/'):
    if file.endswith('.c'):
        ...
        if conf == 'release':
            if arch == 'x64':
                if os.system('gcc -Iinclude -I"' + script_folder + '/include" -Os -nostdlib -DWIN64 ' + path + '/src/' + file + '.c -o ' + path + '/target/x64/release/' + file + '.s -S -masm=intel') != 0:
                    exit(1)
            else:
                if os.system('gcc -Iinclude -I"' + script_folder + '/include" -Os -nostdlib -m32 -mno-sse ' + path + '/src/' + file + '.c -o ' + path + '/target/x86/release/' + file + '.s -S -masm=intel') != 0:
                    exit(1)
        else:
            if arch == 'x64':
                if os.system('gcc -Iinclude -I"' + script_folder + '/include" -Wall -Wextra -Wpedantic -Werror --std=c99 -nostdlib -DWIN64 -DDEBUG ' + path + '/src/' + file + '.c -o ' + path + '/target/x64/debug/' + file + '.s -S -masm=intel') != 0:
                    exit(1)
            else:
                if os.system('gcc -Iinclude -I"' + script_folder + '/include" -Wall -Wextra -Wpedantic -Werror --std=c99 -nostdlib -DDEBUG -m32 -mno-sse ' + path + '/src/' + file + '.c -o ' + path + '/target/x86/debug/' + file + '.s -S -masm=intel') != 0:
                    exit(1)
        assembly_files.append(path + '/target/' + arch + '/' + conf + '/' + file + '.s')
